...
Declaring those method's on `app.ts` before the `run()` function will detect if that is the first run of you app and will clear all data du to `removeAll` and `removeAllSync`
---
**NOTE**
...
secureStorage.doOnFirstRun().then(
  function(success) {
      console.log(`Is that the first rue ? : ${success}`);
  }
);
```
